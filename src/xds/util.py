...
def to_mime(request, documents):
    ...
    payload.write('\n')
    payload.write('Accept-Encoding: gzip,deflate\n')
    payload.write('MIME-Version: 1.0\n')
    payload.write('SOAPAction: "urn:ihe:iti:2007:ProvideAndRegisterDocumentSet-b"\n')
    payload.write('Content-Type: application/xop+xml; charset="UTF-8"; type="application/soap+xml"\n')
    payload.write('Content-Transfer-Encoding: binary\n')
    payload.write('Content-Length: {}\n'.format(len(request)))
